...
if ($result = mysqli_query($connection, $sqlUsuarios)) {
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (($t1 + $t2) > 1) {
            echo "<p class='parrafo-miembros-social'>$nombreUsuario y tú os seguís mutuamente ";
        } 
        // Caso 2. Usuario logeado sigue al otro
        elseif ($t1) {
            echo "<p class='parrafo-miembros-social'>$nombreUsuario te sigue ";
        } 
        // Caso 3. El otro usuario le sigue
        elseif ($t2) {
            echo "<p class='parrafo-miembros-social'>Estás siguiendo a $nombreUsuario ";
        } 
        // Caso 4. No se siguen mutuamente
        else {
            echo "<p class='parrafo-miembros-social'>$nombreUsuario y tú no os estáis siguiendo ";
        }
